gapeR2 <- function(data1, data2 = NULL, NDay = 10, lightdata = NULL, treatment = NULL,
                  coord = NULL, freq1 = 2, freq2 = 300, 
                  exp.days = TRUE, dates = TRUE, blackwhite = FALSE, fixLYaxis = NULL, fixRYaxis = NULL,
    ...
    if(blackwhite == FALSE){
        ...
        indvcolors1 <- c("darkred", "deepskyblue", "darkseagreen", "darkgoldenrod1", "burlywood4", "aquamarine",
                         ...
                         "orange", "gray51", "indianred", "thistle3","darkred", "deepskyblue", "darkseagreen", "darkgoldenrod1", "burlywood4", "aquamarine",
                         "darkorchid", "cyan", "lightblue", "darkslategray", "royalblue1", "maroon",
                         "lightgoldenrod", "darkolivegreen1", "chocolate1", "bisque4", "hotpink", 
                         "lightskyblue", "red", "darkturquoise", "forestgreen", "yellow3", "darkseagreen2",
                         "darkmagenta", "darkblue", "aquamarine4", "green", "blueviolet", "gold", "yellow",
        ...
        if (!is.null(data2)){
            senscolors1 <- c("lightskyblue", "red", "darkturquoise", "forestgreen", "yellow3", "darkseagreen2",
                             ...
                             "lightgoldenrod", "darkolivegreen1", "chocolate1", "bisque4", "hotpink","darkred", "deepskyblue", "darkseagreen", "darkgoldenrod1", "burlywood4", "aquamarine",
                             "darkorchid", "cyan", "lightblue", "darkslategray", "royalblue1", "maroon",
                             "lightgoldenrod", "darkolivegreen1", "chocolate1", "bisque4", "hotpink", 
                             "lightskyblue", "red", "darkturquoise", "forestgreen", "yellow3", "darkseagreen2",
                             "darkmagenta", "darkblue", "aquamarine4", "green", "blueviolet", "gold", "yellow",
    ...
    NDay = 86400*NDay
    Rday <- seq(min(data1[,1]), max(data1[,1]), by = 86400)
    Inicialday <- 1
    ...
    sebasplot <- function(data1) {
        ...
        year <- as.numeric(year(data1[1,1]))
        ...
        minday <- min(Rplotday[1]) ## Minimum julian day value for the plot
        maxday <- max(Rplotday[2]) ## Maximum julian day value for the plot
        
        
        par(mai = c(mai[1],mai[2],mai[3],mai[4]))
        plot(Rplotday, Rsensor, type = "n", axes = FALSE,
             xlab = "", ylab = "", ylim = c(minLeftaxis2, maxLeftaxis2), xlim = c(minday, maxday))
        ...
        if(NDay == 20*86400){
            selecday <- seq(round_any(minJD, 1, f =floor), round_any(xday, 1, f = ceiling), by = 86400*2)
            
            mymonths <- monthnames[month(selecday)]
            mydays <- as.numeric(format(selecday, format="%d"))
            mydate <- paste(sep=" ", mymonths, mydays)
            leapyear <- is.leapyear(year(selecday))
            myexpdays <- yday(selecday) - yday(data1[1,1])+1 + 365*year(selecday)-year + leapyear
            
            if (dates == TRUE & exp.days == FALSE){
                axis(1, at=selecday, labels = as.character(selecday), cex.axis = cex_axis, line = linevaluesX, lwd = lwdaxis, padj = padj)
            }
            if (exp.days == TRUE & dates == FALSE){
                axis(1, at=selecday, labels= myexpdays, cex.axis = cex_axis, line = linevaluesX, lwd = lwdaxis, padj = padj)
            } 
            if (exp.days == TRUE & dates == TRUE){
                dateandexpdat <- paste(as.character(selecday), " ", "(", myexpdays, ")", sep = "")
                axis(1, at=selecday, labels = dateandexpdat, cex.axis = cex_axis, line = linevaluesX, lwd = lwdaxis, padj = padj)
                
            }
            
        } else if(NDay == 86400*10) {
            selecday <- seq(round_any(minJD, 1, f =floor), round_any(xday, 1, f = ceiling), by = 86400*2)
        ...
        } else if(NDay == 30*86400) {
            selecday <- seq(round_any(minJD, 1, f =floor), round_any(xday, 1, f = ceiling), by = 86400*3)
        ...
        } else if(NDay == 71*86400) {
            selecday <- seq(round_any(minJD, 1, f =floor), round_any(xday, 1, f = ceiling), by = 86400*5)
        ...
        if (dates == TRUE & exp.days == FALSE){
            mtext("Dates", side = 1, line = linemtextX[1], font = mtextfont, cex = mtextcex)
        }
        if (exp.days == TRUE & dates == FALSE){
            mtext("Experimental days", side = 1, line = linemtextX[1], font = mtextfont, cex = mtextcex)
        } 
        if (exp.days == TRUE & dates == TRUE){
            mtext("Dates (Exp. days)", side = 1, line = linemtextX[1], font = mtextfont, cex = mtextcex)
        }
        ...
        if (!is.null(data2)){
            ...
            plot(addplotday, Rsensor, type = "n", axes = FALSE,
                 xlab = "", ylab = "", ylim = c(minRightaxis2, maxRightaxis2), xlim = c(minday, maxday))
            
            
            if(any(sensors)){
                axis(side = 4, col = axiscolor(Y1), las = 1, 
                     at = seq(minRightaxis2, maxRightaxis2, by = round_any((maxRightaxis2-minRightaxis2)/10, 0.1, f = floor)), 
                     cex.axis = cex_axis, lwd = lwdaxis)
            }
    ...
    while(TRUE) {  ######  main while loop morebuttons y labs
        ...
        if(K[Nclick] == match("Text", allbuttons, nomatch = NOLAB))
        {
            ...
            for(i in 1:length(nlocators)){
                ...
                date <- as.POSIXct(first$x, origin = '1970-01-01', tz = "GMT")
                leapy <- is.leapyear(year(date))

                
                if (dates == TRUE & exp.days == FALSE){
                    text(x = second$x, y = second$y, labels = paste("x =", as.POSIXct(first$x, origin = '1970-01-01', tz = "GMT"),
                                                                    "y =", round_any(first$y, accuracy = 0.01)))
                    print(paste("x =", as.POSIXct(first$x, origin = '1970-01-01', tz = "GMT")))
                    print(paste("y =", round_any(first$y, accuracy = 0.01)))
                    }
                if (exp.days == TRUE & dates == FALSE){
                    myexpday <- yday(date) - yday(data1[1,1])+1 + 365*(year(date)-year) + leapy
                    text(x = second$x, y = second$y, labels = paste("x =", myexpday,
                                                                    "y =", round_any(first$y, accuracy = 0.01)))
                    print(paste("x =", myexpday))
                    print(paste("y =", round_any(first$y, accuracy = 0.01)))                
                    } 
                if (exp.days == TRUE & dates == TRUE){
                    myexpday <- yday(date) - yday(data1[1,1])+1 + 365*(year(date)-year) + leapy
                    text(x = second$x, y = second$y, labels = paste("x = ", as.POSIXct(first$x, origin = '1970-01-01', tz = "GMT"), " ", "(", myexpday, ");",
                                                                    " y = ", round_any(first$y, accuracy = 0.01), sep = ""))
                    print(paste("x = ", as.POSIXct(first$x, origin = '1970-01-01', tz = "GMT"), " ", "(", myexpday, ")", sep = ""))
                    print(paste("y =", round_any(first$y, accuracy = 0.01)))                
                    }
        ...
        if(K[Nclick] == match("NDay", allbuttons, nomatch = NOLAB))
        {
            ...
            maxNday2 <- round_any(as.numeric(max(data1[,1])-min(data1[,1])), 1, ceiling)
        ...
        {
            mind <- min(Rday) ## Minimum julian day value for the plot
            maxd <- max(Rday) ## Maximum julian day value for the plot
            ...
            if(exp.days == FALSE){
                ...
                validGOTO <- function(){
                    cat("###########################################\n")
                    cat("To what date do you want to go?:\n")
                    cat("First day: ", as.character(mind), "; Last day: ", as.character(maxd), sep = "","\n")
                    cat("Format: year/month/day\n")
                ...
                repeat {
                    newGoTo <- validGOTO()
                    GoTo <- try(as.POSIXct(newGoTo, tz = "GMT"), silent = TRUE)
                    
                    if(is(GoTo, "try-error")){
                        print("wrong date input, try again")
                    } else if(GoTo < min(data1$Date) | GoTo > max(data1$Date)) {
                        print("day out of limits, try again")
                    } else {
                        minJD <- GoTo
                        xday <- minJD + NDay
                        break}
            ...
            if(exp.days == TRUE){
                year <- as.numeric(year(data1[1,1]))
                leapyearGOTO <- is.leapyear(year(Rday))
                myexpdaysGOTO <- yday(Rday) - yday(data1[1,1])+1 + 365*(year(Rday)-year) + leapyearGOTO
                
                cat("###########################################\n")
                cat("To what exp. day do you want to go?:\n")
                cat("First exp.day", min(myexpdaysGOTO), "Last exp.day", max(myexpdaysGOTO))
                newGoTo = readline(prompt = "Type the exp.day: ")
                
                GoTo <- as.numeric(newGoTo)
                
                if(GoTo < min(myexpdaysGOTO) | GoTo > max(myexpdaysGOTO)){
                    cat("No such day")
                } else {
                    minJD <- Rday[GoTo]
                    xday <- minJD + NDay
