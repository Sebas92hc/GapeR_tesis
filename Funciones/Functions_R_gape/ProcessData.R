ProcessData <- function(dataofIntfPinna, dataofprofilux){
...
#    dataindividuals <- data.frame(julday, datost2, datosinterf$Pinna.1, datosinterf$Pinna.2, datosinterf$Pinna.3, datosinterf$Pinna.4,
 #                                 datosf2, datosinterf$Pinna.6, datosinterf$Pinna.7, datosinterf$Pinna.8)
  #  colnames(dataindividuals) <- c("Julianday", "Date", "Time", "Pinna.1A", "Pinna.2A", 
   #                                "Pinna.3A", "Pinna.4A", "Pinna.1B", 
    ...
    ##Convierto la columna de fecha en días julianos
    x1 <- do.call(rbind, strsplit(datos$Date, split="/"))
    date <- as.data.frame(x1); colnames(date) = c("Day", "Month", "Year")
    d <- as.numeric(as.character(date$Day))
    m <- as.numeric(as.character(date$Month)) ## Factor hay que convertirlo primero a caracter y luego a número
    y <- as.numeric(as.character(date$Year))
    jd <- getjul(year = y, month = m, day = d)
    
    ##Convierto la columna de hora en días julianos                   
    x2 <- do.call(rbind, strsplit(datos$Time, split=":"))
    time <- as.data.frame(x2); colnames(time) = c("Hour", "Minute", "Second")
    h <- as.numeric(as.character(time$Hour))
    m <- as.numeric(as.character(time$Minute)) 
    s <- as.numeric(as.character(time$Second))
    
    
    julday <- jd + h/24 + m/(24*60) + s/(24*3600)
    datjd <- data.frame(julday, datos)
    colnames(datjd) <- c("Julianday", "Date", "Time", "pH.A", "Temp.A", "Sal.A", 
                         "Ox.A", "pH.B", "Temp.B", "Sal.B", "Ox.B")
    ...
    datasensors2 <- data.frame(dataindividuals$Julianday, tempsens)
    colnames(datasensors2) <- c("Julianday", "Temp.AF", "Temp.BF")
    ...
    datasensors <- merge.data.frame(x = datjd, y = datasensors2, by = "Julianday", all.x = TRUE, all.y = TRUE)
    ...
    #colnames(datasensors) <- c("Julianday", "Date", "Time", "pH.A", "Temp.A", "Sal.A", 
    #                     "Ox.A", "pH.B", "Temp.B", "Sal.B", "Ox.B", "Temperature.AF", 
    #                     "Temperature.BF")
